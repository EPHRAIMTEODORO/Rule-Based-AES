...
USER_PROMPT_TEMPLATE = """Score this student ESL essay on five traits.

Use integer scores from 1 to 6:
1 = very limited
2 = weak
3 = developing
4 = generally adequate
5 = strong
6 = excellent
...
- If evidence is mixed, choose the lower score.
- Return JSON only.
...
def main() -> None:
    ...
    if args.resume and output_path.exists():
        ...
        if args.essay_id_column in existing_df.columns:
            completed_mask = existing_df["llm_organization_coherence"].notna()
            completed_rows = existing_df.loc[
                completed_mask,
                [args.essay_id_column, *LLM_TRAIT_COLUMNS],
            ]
